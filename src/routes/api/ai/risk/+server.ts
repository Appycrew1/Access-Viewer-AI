...
export const POST:RequestHandler = async ({ request }) => {
  ...
  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model:OPENAI_MODEL, temperature:0.2, response_format:{type:'json_object'}, messages:[{role:'system',content:`You are a safety analyst. Return STRICT JSON: risk_level, flags[], checklist[].`},{role:'user',content:user}] })
  });
