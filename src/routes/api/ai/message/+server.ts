...
export const POST:RequestHandler = async ({ request }) => {
  ...
  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model:OPENAI_MODEL, temperature:0.2, response_format:{type:'json_object'}, messages:[{role:'system',content:`You are a customer comms assistant. Return STRICT JSON: channels[], templates{{email_confirm{{subject,body}}, sms_eta}}.`},{role:'user',content:user}] })
  });
