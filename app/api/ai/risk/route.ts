...
export async function POST(req:Request){
  ...
  "checklist": [
    {
      "item": "Confirm parking waiver",
      "status": "pending"
    },
    {
      "item": "Walk route to entrance",
      "status": "pending"
    },
    {
      "item": "Photograph condition before move",
      "status": "pending"
    }
  ]
}
  if(!OPENAI_API_KEY) return NextResponse.json({ source:'mock', ...mock })
  const sys = `You are an expert operations assistant for a London moving company. Return STRICT JSON for the risk task.`
