...
export const POST:RequestHandler = async ({ request }) => {
  ...
  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model:OPENAI_MODEL, temperature:0.2, response_format:{type:'json_object'}, messages:[{role:'system',content:`You are a quoting assistant. Return STRICT JSON: price_gbp, line_items[], terms[], email_subject, email_body.`},{role:'user',content:user}] })
  });
